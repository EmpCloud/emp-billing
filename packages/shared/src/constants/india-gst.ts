...
export const GST_CESS_RATES: Record<number, number[]> = {
  28: [1, 3, 5, 12, 15, 22, 36, 65], // cess options for 28% slab
};

export function computeGST(amount: number, rate: number, isInterState: boolean) {
...
export const TDS_RATES_INDIA: Record<string, { section: string; rate: number; threshold: number }> = {
...
export function isInterStateGST(sellerStateCode: string, buyerStateCode: string): boolean {
