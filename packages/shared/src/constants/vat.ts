 ...
 */
export const EU_VAT_RATES: Record<string, VATRateSet> = {
 ...
 */
export const UK_VAT_RATES: VATRateSet = {
  standard: 20,
  reduced: [5],
  zero: 0,
};
...
export function getVATRate(
  ...
  const rateSet = upper === "GB" ? UK_VAT_RATES : EU_VAT_RATES[upper];
...
export function isReverseChargeApplicable(
  ...
  const euAndUk = [...EU_COUNTRY_CODES, "GB"];
