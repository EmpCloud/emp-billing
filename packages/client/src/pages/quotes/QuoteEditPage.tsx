...
import { useQuote, useUpdateQuote } from "@/api/hooks/quote.hooks";
...
export function QuoteEditPage() {
  ...
  const taxRates = (taxRatesData?.data ?? []) as {
    id: string;
    name: string;
    rate: number;
  }[];
  ...
  /* Populate form when quote data arrives */
  useEffect(() => {
    if (!quote) return;
    ...
    reset({
      ...
      issueDate: q.issueDate
        ? dayjs(q.issueDate as string).format("YYYY-MM-DD")
        : "",
      expiryDate: q.expiryDate
        ? dayjs(q.expiryDate as string).format("YYYY-MM-DD")
        : "",
      ...
      items: items.length
        ...
        : [{ name: "", description: "", quantity: 1, rate: 0, sortOrder: 0 }],
    });
  }, [quote, reset]);

  /* Totals (display units, not paise) */
  ...
  function onSubmit(values: FormValues) {
    ...
    updateQuote.mutate(payload as unknown as Record<string, unknown>, {
    ...
    <div className="p-6 max-w-6xl">
      <PageHeader
        ...
        breadcrumb={[
          ...
          { label: (quote as unknown as Record<string, unknown>).quoteNumber as string ?? `Quote ${id}`, href: `/quotes/${id}` },
          { label: "Edit" },
        ]}
        actions={
          <Button
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-left">
                  ...
                  <th className="px-3 pb-2 font-medium text-gray-500 text-right">
                    Amount
                  </th>
          ...
          <div className="flex justify-end mt-4">
            <div className="w-64 space-y-2 text-sm">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal</span>
                <span>
                  {formatMoney(Math.round(subtotal * 100), currency)}
                </span>
              </div>
              {taxTotal > 0 && (
                <div className="flex justify-between text-gray-600">
                  <span>Tax</span>
                  <span>
                    {formatMoney(Math.round(taxTotal * 100), currency)}
                  </span>
                </div>
              )}
        ...
        <div className="flex items-center gap-3">
          <Button
            type="submit"
            loading={isSubmitting || updateQuote.isPending}
