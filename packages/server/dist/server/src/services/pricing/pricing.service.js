...
exports.getTieredPriceBreakdown = getTieredPriceBreakdown;
...
function calculatePrice(product, quantity) {
    switch (product.pricingModel) {
        case shared_1.PricingModel.FLAT:
            return Math.round(product.rate * quantity);
        case shared_1.PricingModel.PER_SEAT:
            return Math.round(product.rate * quantity);
        case shared_1.PricingModel.TIERED:
            return calculateTieredPrice(product.pricingTiers ?? [], quantity);
        case shared_1.PricingModel.VOLUME:
            return calculateVolumePrice(product.pricingTiers ?? [], quantity);
        case shared_1.PricingModel.METERED:
 ...
 */
function calculateVolumePrice(tiers, quantity) {
    if (tiers.length === 0)
        return 0;
    // Sort tiers by upTo (null = infinity goes last)
    const sorted = [...tiers].sort((a, b) => {
 ...
 */
function getTieredPriceBreakdown(tiers, quantity) {
    if (tiers.length === 0)
        return [];
    const sorted = [...tiers].sort((a, b) => {
        if (a.upTo === null)
...
// ── Usage records ─────────────────────────────────────────────────────────────
async function recordUsage(orgId, data) {
    const db = await (0, index_1.getDB)();
    // Validate product exists
    const product = await db.findById("products", data.productId, orgId);
    if (!product)
    ...
    if (product.pricingModel !== shared_1.PricingModel.METERED) {
        throw (0, AppError_1.BadRequestError)("Usage records can only be created for metered products");
    }
    const now = new Date();
    const record = await db.create("usage_records", {
        id: (0, uuid_1.v4)(),
        orgId,
...
async function getUsageSummary(orgId, productId, clientId, periodStart, periodEnd) {
    const db = await (0, index_1.getDB)();
    // Get the product for pricing calculation
    const product = await db.findById("products", productId, orgId);
    if (!product)
        throw (0, AppError_1.NotFoundError)("Product");
    ...
    const records = await db.raw(`SELECT COALESCE(SUM(quantity), 0) as total_qty, COUNT(*) as record_count
     FROM usage_records
     WHERE org_id = ? AND product_id = ? AND client_id = ?
       AND period_start >= ? AND period_end <= ?`, [orgId, productId, clientId, periodStart, periodEnd]);
...
async function listUsageRecords(orgId, opts) {
    const db = await (0, index_1.getDB)();
    ...
    const result = await db.findPaginated("usage_records", {
        where,
        page: opts.page,
        limit: opts.limit,
    ...
    if (opts.periodStart || opts.periodEnd) {
        result.data = result.data.filter((r) => {
            ...
            if (opts.periodStart && end < opts.periodStart)
                return false;
            if (opts.periodEnd && start > opts.periodEnd)
                return false;
            return true;
        });
    }
