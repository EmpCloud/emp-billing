...
async function getDashboardStats(orgId) {
    ...
    const statusCounts = await db.raw(`SELECT status, COUNT(*) as count FROM invoices WHERE org_id = ? GROUP BY status`, [orgId]);
    const countMap = {};
    for (const row of statusCounts) {
        countMap[row.status] = Number(row.count);
    }
    // Total revenue (sum of paid invoices)
    const [revenueRow] = await db.raw(`SELECT COALESCE(SUM(total), 0) as total FROM invoices WHERE org_id = ? AND status = ?`, [orgId, shared_1.InvoiceStatus.PAID]);
    const totalRevenue = Number(revenueRow?.total ?? 0);
    // Total outstanding (amount_due on non-void, non-written-off, non-paid invoices)
    const [outstandingRow] = await db.raw(`SELECT COALESCE(SUM(amount_due), 0) as total FROM invoices
     WHERE org_id = ? AND status NOT IN (?, ?, ?)`, [orgId, shared_1.InvoiceStatus.PAID, shared_1.InvoiceStatus.VOID, shared_1.InvoiceStatus.WRITTEN_OFF]);
    const totalOutstanding = Number(outstandingRow?.total ?? 0);
    // Total overdue
    const [overdueRow] = await db.raw(`SELECT COALESCE(SUM(amount_due), 0) as total FROM invoices
     WHERE org_id = ? AND status = ?`, [orgId, shared_1.InvoiceStatus.OVERDUE]);
    const totalOverdue = Number(overdueRow?.total ?? 0);
    // Total expenses
    const [expenseRow] = await db.raw(`SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE org_id = ?`, [orgId]);
    const totalExpenses = Number(expenseRow?.total ?? 0);
    // Recent 5 invoices
    const recentInvoices = await db.raw(`SELECT * FROM invoices WHERE org_id = ? ORDER BY created_at DESC LIMIT 5`, [orgId]);
    // Recent 5 payments
    const recentPayments = await db.raw(`SELECT * FROM payments WHERE org_id = ? AND is_refund = false ORDER BY date DESC LIMIT 5`, [orgId]);
    // Receivables aging
    const agingBuckets = await db.raw(`SELECT
       CASE
     ...
     GROUP BY bucket`, [orgId, shared_1.InvoiceStatus.PAID, shared_1.InvoiceStatus.VOID, shared_1.InvoiceStatus.WRITTEN_OFF]);
...
// ── Revenue Report ──────────────────────────────────────────────────────────
async function getRevenueReport(orgId, from, to) {
    const db = await (0, index_1.getDB)();
    const rows = await db.raw(`SELECT DATE_FORMAT(paid_at, '%Y-%m') as month, COALESCE(SUM(total), 0) as revenue
     FROM invoices
     WHERE org_id = ? AND status = ? AND paid_at >= ? AND paid_at <= ?
     GROUP BY month
     ORDER BY month ASC`, [orgId, shared_1.InvoiceStatus.PAID, from, to]);
    return { data: rows.map((r) => ({ month: r.month, revenue: Number(r.revenue) })) };
}
// ── Receivables Report ──────────────────────────────────────────────────────
async function getReceivablesReport(orgId) {
    const db = await (0, index_1.getDB)();
    const rows = await db.raw(`SELECT i.client_id, c.name as client_name,
     ...
     ORDER BY total_outstanding DESC`, [orgId, shared_1.InvoiceStatus.PAID, shared_1.InvoiceStatus.VOID, shared_1.InvoiceStatus.WRITTEN_OFF]);
    return {
        data: rows.map((r) => ({
...
// ── Aging Report ────────────────────────────────────────────────────────────
async function getAgingReport(orgId) {
    const db = await (0, index_1.getDB)();
    const rows = await db.raw(`SELECT
     ...
     WHERE i.org_id = ? AND i.status NOT IN (?, ?, ?)
     GROUP BY i.client_id, c.name
     ORDER BY c.name ASC`, [orgId, shared_1.InvoiceStatus.PAID, shared_1.InvoiceStatus.VOID, shared_1.InvoiceStatus.WRITTEN_OFF]);
    return {
        data: rows.map((r) => ({
            clientId: r.client_id,
            clientName: r.client_name,
...
// ── Expense Report ──────────────────────────────────────────────────────────
async function getExpenseReport(orgId, from, to) {
    const db = await (0, index_1.getDB)();
    const rows = await db.raw(`SELECT e.category_id, ec.name as category_name,
     ...
     ORDER BY total DESC`, [orgId, from, to]);
    return {
        data: rows.map((r) => ({
...
// ── Profit & Loss Report ────────────────────────────────────────────────────
async function getProfitLossReport(orgId, from, to) {
    const db = await (0, index_1.getDB)();
    // Monthly revenue from paid invoices
    const revenueRows = await db.raw(`SELECT DATE_FORMAT(paid_at, '%Y-%m') as month, COALESCE(SUM(total), 0) as total
     FROM invoices
     WHERE org_id = ? AND status = ? AND paid_at >= ? AND paid_at <= ?
     GROUP BY month
     ORDER BY month ASC`, [orgId, shared_1.InvoiceStatus.PAID, from, to]);
    // Monthly expenses
    const expenseRows = await db.raw(`SELECT DATE_FORMAT(date, '%Y-%m') as month, COALESCE(SUM(amount), 0) as total
     FROM expenses
     WHERE org_id = ? AND date >= ? AND date <= ?
     GROUP BY month
     ORDER BY month ASC`, [orgId, from, to]);
...
// ── Tax Report ─────────────────────────────────────────────────────────
async function getTaxReport(orgId, from, to) {
    const db = await (0, index_1.getDB)();
    // Build date filter clause for invoice issue_date
    const dateConditions = ["i.org_id = ?", "i.status NOT IN (?, ?)"];
    const params = [orgId, shared_1.InvoiceStatus.VOID, shared_1.InvoiceStatus.WRITTEN_OFF];
    if (from) {
    ...
    const rows = await db.raw(`SELECT
     ...
     ORDER BY ii.tax_rate ASC`, params);
...
// ── Top Clients ─────────────────────────────────────────────────────────────
async function getTopClients(orgId, from, to, limit = 10) {
    const db = await (0, index_1.getDB)();
    const rows = await db.raw(`SELECT i.client_id, c.name as client_name,
     ...
     LIMIT ?`, [orgId, shared_1.InvoiceStatus.PAID, from, to, limit]);
    return {
        data: rows.map((r) => ({
            clientId: r.client_id,
            clientName: r.client_name,
