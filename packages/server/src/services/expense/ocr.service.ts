...
// ============================================================================
// OCR SERVICE — Receipt scanning with pluggable providers
// ============================================================================
...
export class TesseractOCRProvider implements IOCRProvider {
  ...
  async extractText(imageBuffer: Buffer, mimeType: string): Promise<OCRResult> {
    try {
      ...
      const Tesseract: any = await import(/* @vite-ignore */ "tesseract.js");
      ...
      if (!validMimeTypes.includes(mimeType)) {
        logger.warn(`Unsupported mime type for Tesseract OCR: ${mimeType}`);
      ...
      logger.error("Tesseract OCR extraction failed", { err });
      return { rawText: "", confidence: 0 };
    }
  }
}
...
export class CloudOCRProvider implements IOCRProvider {
  readonly name = "cloud";

  private readonly provider: "google-vision" | "aws-textract" | "azure-form";

  constructor(provider: "google-vision" | "aws-textract" | "azure-form" = "google-vision") {
  ...
  async extractText(imageBuffer: Buffer, mimeType: string): Promise<OCRResult> {
    ...
    logger.warn(
      `CloudOCRProvider (${this.provider}) is a placeholder. ` +
...
function extractMerchantName(lines: string[]): string | null {
  ...
  for (const line of lines.slice(0, 5)) {
    ...
    if (/^(total|subtotal|sub\s*total|amount|balance|change|cash|card)/i.test(line)) continue;
...
function extractTotal(text: string): { total: number | null; currency: string | null } {
...
export async function processReceipt(
  ...
  logger.info(`Processing receipt with OCR provider: ${provider.name}`);
