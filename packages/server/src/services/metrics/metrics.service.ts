...
export async function getMRR(orgId: string): Promise<MRRMetrics> {
  const db = await getDB();

  // Current MRR: sum of active subscription prices normalized to monthly
  const currentRows = await db.raw<{
    price: number;
    billing_interval: string;
    quantity: number;
  }[]>(
    `SELECT p.price, p.billing_interval, s.quantity
     FROM subscriptions s
     JOIN plans p ON p.id = s.plan_id
     WHERE s.org_id = ? AND s.status = ?`,
    [orgId, SubscriptionStatus.ACTIVE],
  );

  const mrr = currentRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );

  // Last month MRR: subscriptions that were active one month ago
  const oneMonthAgo = dayjs().subtract(1, "month").format("YYYY-MM-DD HH:mm:ss");
  const lastMonthRows = await db.raw<{
    price: number;
    billing_interval: string;
    quantity: number;
  }[]>(
    `SELECT p.price, p.billing_interval, s.quantity
     ...
     WHERE s.org_id = ?
       AND s.created_at <= ?
       AND (s.status = ? OR (s.cancelled_at IS NOT NULL AND s.cancelled_at > ?))`,
    [orgId, oneMonthAgo, SubscriptionStatus.ACTIVE, oneMonthAgo],
  );

  const lastMrr = lastMonthRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );
...
export async function getARR(orgId: string): Promise<ARRMetrics> {
...
export async function getChurnMetrics(
  ...
  const [activeStartRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscriptions
     WHERE org_id = ? AND created_at <= ? AND (status = ? OR (cancelled_at IS NOT NULL AND cancelled_at > ?))`,
    [orgId, from, SubscriptionStatus.ACTIVE, from],
  );
  const activeAtStart = Number(activeStartRow?.count ?? 0);

  // Cancelled during period
  const [cancelledRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscriptions
     WHERE org_id = ? AND cancelled_at >= ? AND cancelled_at <= ?`,
    [orgId, from, to],
  );
  ...
  // MRR at start
  const mrrStartRows = await db.raw<{
    price: number;
    billing_interval: string;
    quantity: number;
  }[]>(
  ...
  const mrrAtStart = mrrStartRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );

  // MRR lost from cancellations
  const churnedRows = await db.raw<{
    price: number;
    billing_interval: string;
    quantity: number;
  }[]>(
    `SELECT p.price, p.billing_interval, s.quantity
     FROM subscriptions s
     JOIN plans p ON p.id = s.plan_id
     WHERE s.org_id = ? AND s.cancelled_at >= ? AND s.cancelled_at <= ?`,
    [orgId, from, to],
  );
  const mrrLost = churnedRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );
  ...
  const mrrEndRows = await db.raw<{
    ...
    `SELECT p.price, p.billing_interval, s.quantity
     FROM subscriptions s
     JOIN plans p ON p.id = s.plan_id
     WHERE s.org_id = ? AND s.status = ? AND s.created_at <= ?`,
    [orgId, SubscriptionStatus.ACTIVE, to],
  );
  const mrrAtEnd = mrrEndRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );
...
export async function getLTV(orgId: string): Promise<LTVMetrics> {
  const db = await getDB();

  // Average revenue per customer (monthly)
  const activeRows = await db.raw<{
    price: number;
    billing_interval: string;
    quantity: number;
  }[]>(
  ...
  const totalMonthlyRevenue = activeRows.reduce(
    (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
    0,
  );
  ...
  const durationRows = await db.raw<{ avg_months: number }[]>(
    `SELECT AVG(
  ...
  const threeMonthsAgo = dayjs().subtract(3, "month").format("YYYY-MM-DD HH:mm:ss");
  const [cancelledRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscriptions
     WHERE org_id = ? AND cancelled_at >= ?`,
    [orgId, threeMonthsAgo],
  );
  const [totalRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscriptions WHERE org_id = ?`,
    [orgId],
  );
...
export async function getRevenueBreakdown(
  orgId: string,
  months: number = 12,
): Promise<RevenueBreakdownMonth[]> {
  const db = await getDB();
  const result: RevenueBreakdownMonth[] = [];
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const newRows = await db.raw<{
      ...
      `SELECT p.price, p.billing_interval, s.quantity
       ...
       WHERE s.org_id = ? AND s.created_at >= ? AND s.created_at <= ?
         AND s.status IN (?, ?)`,
      [orgId, monthStart.format("YYYY-MM-DD"), monthEnd.format("YYYY-MM-DD 23:59:59"),
       SubscriptionStatus.ACTIVE, SubscriptionStatus.TRIALING],
    );
    const newMRR = newRows.reduce(
      (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
      0,
    );

    // Expansion MRR: subscriptions upgraded this month (via subscription_events)
    const [expansionRow] = await db.raw<{ total: number }[]>(
      `SELECT COALESCE(SUM(
    ...
    const [contractionRow] = await db.raw<{ total: number }[]>(
      `SELECT COALESCE(SUM(
         (SELECT normalizing_price FROM (
           SELECT p2.price / CASE p2.billing_interval
             WHEN 'monthly' THEN 1 WHEN 'quarterly' THEN 3
    ...
    const churnMRR = churnRows.reduce(
      (sum, r) => sum + normalizeToMonthly(Number(r.price), r.billing_interval) * Number(r.quantity),
      0,
    );
...
export async function getSubscriptionStats(orgId: string): Promise<SubscriptionStats> {
  const db = await getDB();

  const statusCounts = await db.raw<{ status: string; count: number }[]>(
    `SELECT status, COUNT(*) as count FROM subscriptions WHERE org_id = ? GROUP BY status`,
    [orgId],
  );
  ...
  const [trialRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscription_events
     WHERE org_id = ? AND event_type = 'trial_started'`,
    [orgId],
  );
  const [activatedRow] = await db.raw<{ count: number }[]>(
    `SELECT COUNT(*) as count FROM subscription_events
     WHERE org_id = ? AND event_type = 'activated'`,
    [orgId],
  );
  ...
  const totalActive = countMap[SubscriptionStatus.ACTIVE] ?? 0;
  ...
  return {
    totalActive: countMap[SubscriptionStatus.ACTIVE] ?? 0,
    totalTrialing: countMap[SubscriptionStatus.TRIALING] ?? 0,
    totalPaused: countMap[SubscriptionStatus.PAUSED] ?? 0,
    totalPastDue: countMap[SubscriptionStatus.PAST_DUE] ?? 0,
    totalCancelled: countMap[SubscriptionStatus.CANCELLED] ?? 0,
    conversionRate,
...
export async function getCohortAnalysis(
  ...
  for (let i = months - 1; i >= 0; i--) {
    const cohortMonth = dayjs().subtract(i, "month").startOf("month");
    ...
    const [totalRow] = await db.raw<{ count: number }[]>(
      ...
      [orgId, cohortMonth.format("YYYY-MM-DD"), cohortEnd.format("YYYY-MM-DD 23:59:59")],
    );
    ...
    for (let m = 0; m < Math.min(maxMonths, months); m++) {
      ...
      const [retainedRow] = await db.raw<{ count: number }[]>(
        `SELECT COUNT(*) as count FROM subscriptions
         WHERE org_id = ?
