...
// ============================================================================
// e-INVOICE SERVICE (Indian GST e-Invoice)
// Hook / interface layer for e-Invoice generation via the GST IRP portal.
// Actual API credentials and endpoints are configured per organization.
// ============================================================================

// ── Types ───────────────────────────────────────────────────────────────────

/** Result of successfully generating an IRN (Invoice Reference Number) */
export interface EInvoiceIRNResult {
  irn: string;
...
/** Result of cancelling an IRN */
export interface EInvoiceCancelResult {
  success: boolean;
  cancelDate?: string;
}

/** Org-level e-Invoice configuration stored in settings */
export interface EInvoiceConfig {
  enabled: boolean;
  /** IRP API base URL (NIC production or sandbox) */
  apiBaseUrl: string;
  /** GSP (GST Suvidha Provider) client ID */
  gspClientId: string;
  /** GSP client secret */
  gspClientSecret: string;
  /** Seller GSTIN */
  gstin: string;
  /** Username for IRP portal */
  username: string;
  /** Password for IRP portal (encrypted) */
  password: string;
  /** Auto-generate e-Invoice on invoice creation */
  autoGenerate: boolean;
  /** Turnover threshold above which e-Invoice is mandatory (in paise) */
  turnoverThreshold: number;
}

/** Invoice data mapped to e-Invoice JSON schema (simplified) */
...
export interface IEInvoiceProvider {
  authenticate(eInvoiceConfig: EInvoiceConfig): Promise<string>;
  generateIRN(authToken: string, payload: EInvoicePayload, eInvoiceConfig: EInvoiceConfig): Promise<EInvoiceIRNResult>;
  cancelIRN(authToken: string, irn: string, reason: EInvoiceCancelReason, remark: string, eInvoiceConfig: EInvoiceConfig): Promise<EInvoiceCancelResult>;
  getIRNDetails(authToken: string, irn: string, eInvoiceConfig: EInvoiceConfig): Promise<EInvoiceIRNResult | null>;
}

export type EInvoiceCancelReason =
  | "1" // Duplicate
...
export class NICEInvoiceProvider implements IEInvoiceProvider {
  async authenticate(cfg: EInvoiceConfig): Promise<string> {
    ...
    if (!response.ok) {
      ...
      logger.error("e-Invoice authentication failed", { status: response.status, body: errorBody });
      throw new AppError(502, "EINVOICE_AUTH_FAILED", "Failed to authenticate with e-Invoice portal");
    }

    const result = (await response.json()) as { Status: number; Data: { AuthToken: string; TokenExpiry: string } };
    if (result.Status !== 1 || !result.Data?.AuthToken) {
      throw new AppError(502, "EINVOICE_AUTH_FAILED", "e-Invoice portal returned invalid auth response");
    }

    logger.info("e-Invoice authentication successful", { expiry: result.Data.TokenExpiry });
    return result.Data.AuthToken;
  }

  async generateIRN(authToken: string, payload: EInvoicePayload, cfg: EInvoiceConfig): Promise<EInvoiceIRNResult> {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      logger.error("e-Invoice IRN generation failed", { status: response.status, body: errorBody });
      throw new AppError(502, "EINVOICE_GENERATE_FAILED", "Failed to generate IRN from e-Invoice portal");
    }
    ...
    if (result.Status !== 1 || !result.Data?.Irn) {
      ...
      throw new AppError(502, "EINVOICE_GENERATE_FAILED", `e-Invoice generation failed: ${errors}`);
    }

    logger.info("e-Invoice IRN generated", { irn: result.Data.Irn, ackNo: result.Data.AckNo });
  ...
  async cancelIRN(
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      logger.error("e-Invoice IRN cancellation failed", { status: response.status, body: errorBody, irn });
      throw new AppError(502, "EINVOICE_CANCEL_FAILED", "Failed to cancel IRN on e-Invoice portal");
    }
    ...
    if (result.Status !== 1) {
      const errors = result.ErrorDetails?.map((e) => `${e.ErrorCode}: ${e.ErrorMessage}`).join("; ") ?? "Unknown error";
      throw new AppError(502, "EINVOICE_CANCEL_FAILED", `e-Invoice cancellation failed: ${errors}`);
    }

    logger.info("e-Invoice IRN cancelled", { irn, cancelDate: result.Data.CancelDate });
  ...
  async getIRNDetails(authToken: string, irn: string, cfg: EInvoiceConfig): Promise<EInvoiceIRNResult | null> {
    ...
    if (!response.ok) {
      logger.warn("e-Invoice IRN details fetch failed", { status: response.status, irn });
      return null;
    }
...
let einvoiceProvider: IEInvoiceProvider | null = null;

export function getEInvoiceProvider(): IEInvoiceProvider {
  if (!einvoiceProvider) {
...
export function setEInvoiceProvider(provider: IEInvoiceProvider): void {
 ...
 */
export async function onInvoiceCreated(orgId: string, invoiceId: string): Promise<EInvoiceIRNResult | null> {
  const einvoiceConfig = await getEInvoiceConfig(orgId);
  if (!einvoiceConfig || !einvoiceConfig.enabled || !einvoiceConfig.autoGenerate) {
    logger.debug("e-Invoice not enabled or auto-generate off, skipping", { orgId, invoiceId });
    return null;
  }
  ...
  if (!invoice) {
    logger.warn("onInvoiceCreated: invoice not found", { orgId, invoiceId });
    return null;
  }
  ...
  try {
    ...
    logger.info("e-Invoice IRN stored on invoice", { orgId, invoiceId, irn: result.irn });
    return result;
  } catch (err) {
    logger.error("e-Invoice generation failed for invoice", { orgId, invoiceId, err });
    throw err;
  }
}
...
export async function onInvoiceCancelled(
  orgId: string,
  invoiceId: string,
  reason: EInvoiceCancelReason = "3",
  remark = "Invoice cancelled",
): Promise<EInvoiceCancelResult | null> {
  const einvoiceConfig = await getEInvoiceConfig(orgId);
  ...
  const invoice = await db.findOne("invoices", { id: invoiceId, org_id: orgId });
  if (!invoice) {
    logger.warn("onInvoiceCancelled: invoice not found", { orgId, invoiceId });
    return null;
  }

  const inv = invoice as Record<string, unknown>;
  const irn = inv.einvoice_irn as string | undefined;
  if (!irn) {
    logger.debug("onInvoiceCancelled: no IRN on invoice, nothing to cancel", { orgId, invoiceId });
    return null;
  }
  ...
  try {
    ...
    logger.info("e-Invoice IRN cancelled for invoice", { orgId, invoiceId, irn });
    return result;
  } catch (err) {
    logger.error("e-Invoice cancellation failed for invoice", { orgId, invoiceId, irn, err });
    throw err;
  }
}
 ...
 */
export async function generateIRN(orgId: string, invoiceId: string): Promise<EInvoiceIRNResult> {
  const einvoiceConfig = await getEInvoiceConfig(orgId);
  if (!einvoiceConfig || !einvoiceConfig.enabled) {
    throw new AppError(400, "EINVOICE_NOT_ENABLED", "e-Invoice is not enabled for this organization");
  }
  ...
  const inv = invoice as Record<string, unknown>;
  if (inv.einvoice_irn) {
    throw new AppError(409, "CONFLICT", "Invoice already has an IRN generated");
  }
...
export async function cancelIRN(
  ...
  if (!result) {
    throw new AppError(400, "EINVOICE_CANCEL_FAILED", "Could not cancel IRN — e-Invoice may not be enabled or invoice has no IRN");
  }
  return result;
}
...
async function buildEInvoicePayload(orgId: string, invoice: unknown): Promise<EInvoicePayload> {
  const db = await getDB();
  const inv = invoice as Record<string, unknown>;
  ...
  if (!client) throw new AppError(400, "BAD_REQUEST", "Client not found for e-Invoice generation");
