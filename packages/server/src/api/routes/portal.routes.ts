...
import { CreateDisputeSchema } from "@emp-billing/shared";
...
// Public — no auth
router.post("/login", asyncHandler(portalController.portalLogin));
...
router.post("/subscriptions/:id/change-plan", portalAuth, asyncHandler(portalController.portalChangePlan));
router.post("/subscriptions/:id/cancel",  portalAuth, asyncHandler(portalController.portalCancelSubscription));

// Payment method management
router.get("/payment-method",     portalAuth, asyncHandler(portalController.getPortalPaymentMethod));
router.put("/payment-method",     portalAuth, asyncHandler(portalController.updatePortalPaymentMethod));
...
router.post("/pay",                 portalAuth, asyncHandler(onlinePaymentController.createOrder));
router.post("/verify-payment",      portalAuth, asyncHandler(onlinePaymentController.verifyPayment));
