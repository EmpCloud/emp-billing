...
async function findOrCreateClient(
  ...
  await db.create("clients", {
    ...
    email: `org-${empcloudOrgId}@empcloud.internal`,
    phone: null,
    company: `EmpCloud Organization ${empcloudOrgId}`,
    taxId: null,
    billingAddress: null,
  ...
  switch (eventType) {
    case "subscription.created": {
      ...
      await db.create("invoices", {
        ...
        status: "draft",
        issueDate: now,
        dueDate: dayjs(now).add(30, "day").toDate(),
        subtotal: lineTotal,
        taxTotal: 0,
        discountTotal: 0,
        total: lineTotal,
        ...
        amountDue: lineTotal,
        currency: body.currency || "INR",
        notes: `Auto-generated invoice for ${body.module_name || body.module_slug} subscription`,
        metadata: JSON.stringify({ empcloud_org_id: body.organization_id, module_slug: body.module_slug }),
        createdBy: "empcloud-webhook",
        createdAt: now,
        updatedAt: now,
      });
      ...
      await db.create("invoice_items", {
        ...
        invoiceId,
        orgId,
        description: `${body.module_name || body.module_slug} — ${body.plan_tier} (${body.total_seats} seats)`,
        quantity: body.total_seats || 1,
        unitPrice: body.price_per_seat || 0,
        amount: lineTotal,
        sortOrder: 0,
        createdAt: now,
        updatedAt: now,
